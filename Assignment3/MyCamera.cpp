...
{
	//quat to change the camera rotation
	quaternion m_qOrientation;

	//setting the direction vectors
	m_v3Forward = glm::normalize(m_v3Position - m_v3Target);
	m_v3Upward = glm::normalize(m_v3Position - m_v3Above);
	m_v3Right = glm::normalize(glm::cross(m_v3Forward, m_v3Upward));

	//Change the Yaw and the Pitch of the camera
	m_qOrientation = glm::angleAxis(glm::radians(m_v3PitchYaw.y), m_v3Forward) * m_qOrientation;
	m_qOrientation = glm::angleAxis(glm::radians(m_v3PitchYaw.x), m_v3Upward) * m_qOrientation;

	//rotating the forward vector
	m_v3Forward = glm::rotate(m_qOrientation, m_v3PitchYaw);


	//setting the target to the correct rotation
	m_v3Target = m_v3Forward * 10;

	//moving the target based on key input
	m_v3Target += m_v3TargetMove;
...
//function to move the user forward or backward in the world space
void MyCamera::MoveForward(float a_fDistance)
{
	m_v3Position += glm::normalize(m_v3Position - m_v3Target) * -a_fDistance;
	m_v3TargetMove += glm::normalize(m_v3Position - m_v3Target) * -a_fDistance;
	m_v3Above += glm::normalize(m_v3Position - m_v3Target) * -a_fDistance;
}

//function to move the user up or down in the world space
void MyCamera::MoveVertical(float a_fDistance)
{
	m_v3Position += vector3(0, -a_fDistance, 0);
	m_v3TargetMove += vector3(0, -a_fDistance, 0);
	m_v3Above += vector3(0,-a_fDistance,0);
}
//function to move the user left or right in the world space
void MyCamera::MoveSideways(float a_fDistance)

{
	m_v3Position += m_v3Right * -a_fDistance;
	m_v3TargetMove += m_v3Right * -a_fDistance;
	m_v3Above += m_v3Right * -a_fDistance;
	/*m_v3Position += glm::normalize(glm::cross(m_v3Position - m_v3Target, m_v3Position - m_v3Above)) * -a_fDistance;
	m_v3Target += glm::normalize(glm::cross(m_v3Position - m_v3Target, m_v3Position - m_v3Above)) * -a_fDistance;
	m_v3Above += glm::normalize(glm::cross(m_v3Position - m_v3Target, m_v3Position - m_v3Above)) * -a_fDistance;*/
}

//function to change the pitch of the camera
void MyCamera::ChangePitch(float angle)
{
	m_v3PitchYaw.y += angle;
}

//function to change the yaw of the camera
void MyCamera::ChangeYaw(float angle)
{
	m_v3PitchYaw.x += angle;
}